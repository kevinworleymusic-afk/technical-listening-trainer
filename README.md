...
- Tested-parameter capture per trial: tested frequencies, gains, and Q values; trial context including band mode, gain direction, randomization mode, and automobile conditions
...
- Export session to CSV (standard file-save dialog, timestamped default filename, defaults to `results/`; exports scored-answer rows plus appended analytics)
- Analytics rows include total success rate, total parameter-match rate, frequency-by-frequency performance, and strengths/weaknesses across tested parameters
- Runtime-generated audio files are stored in a temporary session directory and cleaned automatically on app close/exit (no persistent generated WAV clutter in project/user folders)
- Legacy generated WAV files from older versions (for example: `modified.wav`, `match_guess.wav`, `memorization.wav`, `selected_section.wav`, `trial_reference.wav`, `trial_modified.wav`) can be safely deleted from the project root
- Reset session stats clears in-memory score/session data for a clean start
